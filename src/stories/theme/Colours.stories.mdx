...
import { ColorSwatch } from "../utils/components";
...
|---|---|---|
| `primary` <ColorSwatch name="Primary" value={colors.primary} size={16} /> | `onPrimary` | Main accent color |
| `back` <ColorSwatch name="Back" value={colors.back} size={16} /> | `onBack` | Base background / foreground color |
| `surface` <ColorSwatch name="Surface" value={colors.surface} size={16} /> | `onSurface` | Colors for raised elements (cards) |
| `error` <ColorSwatch name="Error" value={colors.error} size={16} /> | `onError` | Used for displaying errors |
